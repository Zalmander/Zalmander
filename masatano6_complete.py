...
def loadtxttovar(txt): # used to load text from file into variable
    with open(txt) as f:
        lines = f.read()
    return lines

def b64decode(text): # used
...
def hamm_dist_cipher(ciphertext): # used
    keysize_hd = []
...
def keysizeblocks(ciphertext, keysize): # used
...
def transpose_list(toBeTransposed,m): # used
    transposed = []
...
def sbXOR_dtect_multiple(transList): 
    listCypherChars = []
...
def repeating_key_XOR(unencrypted_str, key):
    counter=0
    encrypted_list = []
...
'''
def hamming_distance(hamstr1,hamstr2):
    text_list1 = [hamstr1[i:i+1] for i in range(0, len(hamstr1), 1)]
    text_list2 = [hamstr2[i:i+1] for i in range(0, len(hamstr2), 1)]
    counter = 0
    for (j,k) in zip(text_list1,text_list2):
        x = (bin(ord(j)^ord(k))[2:])
        for m in x:
            if m == '1':
                counter += 1
